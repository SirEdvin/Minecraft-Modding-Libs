...
abstract class StorageTests {
    ...
    abstract fun createStorage(items: List<ItemStack>, secondary: Boolean): AccessibleAgnosticItemStorage

    fun createStorage(sizes: List<Int>, stack: ItemStack, secondary: Boolean): AccessibleAgnosticItemStorage = createStorage(
        sizes.map {
            if (it == 0) {
                ItemStack.EMPTY
            } else {
