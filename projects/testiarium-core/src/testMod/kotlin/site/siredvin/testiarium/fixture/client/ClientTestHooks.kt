...
object ClientTestHooks {
    ...
    private fun startTests(server: MinecraftServer): MultipleTestTracker? {
        ...
        val tests = GameTestRunner.Builder.fromBatches(
            ...
            .newStructureSpawner(StructureGridSpawner(Testiarium.getTestOrigin(server), 8, false))
            .build()
        return MultipleTestTracker(tests.testInfos).also {
            it.addListener(object : GameTestListener {
                private fun cleanup() = server.playerList.players.forEach(ServerPlayer::setupForTest)

                override fun testPassed(test: GameTestInfo, runner: GameTestRunner) = cleanup()

                override fun testFailed(test: GameTestInfo, runner: GameTestRunner) = cleanup()

                override fun testStructureLoaded(test: GameTestInfo) = Unit

                override fun testAddedForRerun(test: GameTestInfo, newTest: GameTestInfo, runner: GameTestRunner) = Unit
            })
            tests.start()
            tracker = it
        }
