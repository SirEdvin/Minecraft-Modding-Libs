...
object FixtureCommands {
    fun register(dispatcher: CommandDispatcher<CommandSourceStack>, buildContext: CommandBuildContext) {
        dispatcher.register(
            literal("testiarium")
                .then(literal("import").executes { sync(source(), destination()); 1 })
                .then(literal("export").executes { sync(destination(), source()); 1 })
                ...
                .then(literal("marker").executes { context ->
                    ...
                    EntityType.ARMOR_STAND.create(player.level())?.apply {
                        readAdditionalSaveData(CompoundTag().apply {
    ...
    private fun requireProperty(name: String): String =
        System.getProperty(name) ?: error("Set -D$name to use Testiarium fixture commands")
