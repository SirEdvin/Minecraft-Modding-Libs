...
object Testiarium {
    ...
    fun onServerStarted(server: MinecraftServer) {
        ...
        net.minecraft.gametest.framework.StructureUtils.findStructureBlocks(getTestOrigin(server), 200, level).forEach { pos ->
            val structure = level.getBlockEntity(pos) as? net.minecraft.world.level.block.entity.StructureBlockEntity ?: return@forEach
            net.minecraft.gametest.framework.StructureUtils.clearSpaceForStructure(
                net.minecraft.gametest.framework.StructureUtils.getStructureBoundingBox(structure),
                level,
            )
        }
    }
    ...
    fun getTestOrigin(server: MinecraftServer): BlockPos {
        ...
        return BlockPos(spawn.x, -59, spawn.z)
    }
